...
just prettier-fix    # Format TypeScript/React files with Prettier
just eslint-fix      # Auto-fix ESLint issues

# Checking (local)
just prettier-check  # Check Prettier formatting
just eslint-check    # Check ESLint
```
...
`just dev` runs a Docker container with:
